

const Mprestation = () => {
    return (
        <>
        <section id="m-presation" className="about-gallery_block">
        <div className="wrapper">
            <div className="about-gallery_container">
                <div className="about-gallery_text">
                    <h2 className='aboutgallery'>Coiffure homme</h2>
                    <h2 className='aboutgallery-title'>Tarifs de nos prestations Coiffure Homme (A partir de...)</h2>
                    <p className='aboutgallery-txt'>Les tarifs peuvent varier d’un salon de coiffure l’autre. Pour le prix exact des prestations, veuillez visiter la page du salon de coiffure.</p>
                </div>
            </div>
        </div>
        </section> 
        </>
    );
};

export default Mprestation;